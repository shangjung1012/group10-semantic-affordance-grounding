...
\midrule
\texttt{ontology/group-ontology.ttl} & Group 10 authored ontology with task objects, task roles, affordances, and the formal graspability axiom. \\
\texttt{ontology/imports/course-affordance.ttl} & Provided course ontology, with a minimal Turtle syntax repair for \texttt{cap:hasApproxWidth}. \\
\texttt{ontology/imports/course-alignment.ttl} & Provided SKOS alignment resource for semantic traceability. \\
\texttt{ontology/inferred-results.ttl} & Exported graph after reasoning. \\
\texttt{queries/graspable\_objects.rq} & Required SPARQL query for inferred graspable objects. \\
\texttt{queries/task\_objects.rq} & Additional query for all modeled baseline task objects. \\
\texttt{results/graspable\_objects\_output.txt} & Saved output of the required query. \\
\texttt{src/run\_reasoning.py} & Reproducible local reasoning and query workflow. \\
\texttt{tests/test\_hw5\_workflow.py} & Tests for parsing, modeling constraints, inference, and saved outputs. \\
\bottomrule
\end{tabular}
...
\begin{table}[h]
\centering
\begin{tabular}{p{0.23\linewidth} p{0.28\linewidth} p{0.35\linewidth}}
\toprule
\textbf{Layer} & \textbf{Meaning} & \textbf{Examples} \\
\midrule
Object type & Shared class for object category & \texttt{cap:Cup}, \texttt{cap:Knife}, \texttt{cap:ToyBlock} \\
Task role & Role played in a task & \texttt{cap:TargetObject}, \texttt{cap:ReferenceObject} \\
Affordance & Action possibility in context & \texttt{cap:GraspingAffordance}, \texttt{cap:ContainmentAffordance} \\
Instance & Group-specific observed object & \texttt{g10:blueCup01}, \texttt{g10:block01} \\
\bottomrule
\end{tabular}
...
\begin{table}[h]
\centering
\begin{tabular}{p{0.22\linewidth} p{0.18\linewidth} p{0.22\linewidth} p{0.25\linewidth}}
\toprule
...
\begin{enumerate}
    ...
    \item Materialize the assignment-specific graspability rule for physical objects that have a \texttt{cap:GraspingAffordance}.
