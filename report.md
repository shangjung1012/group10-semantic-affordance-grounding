...
Course-level classes and properties are reused from `cap:`. Group-specific individuals, tasks, and reusable affordance individuals are declared under `g10:`.
...
The central class is `cap:GraspableObject`. It is defined in `ontology/group-ontology.ttl` as a physical object that has at least one grasping affordance:
...
2. Applies OWL/RDFS closure with `owlrl`.
3. Materializes the assignment-specific graspability rule for physical objects that have a `cap:GraspingAffordance`.
4. Writes `ontology/inferred-results.ttl`.
5. Runs the SPARQL queries and writes saved outputs under `results/`.
